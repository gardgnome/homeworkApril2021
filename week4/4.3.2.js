...
const data = [
  ...
  let coordsLatLong = data.map(user =>
    {user.adress.geo.lat, user.address.geo.lng}
  );
  console.log(coordsLatLong);

  // create an array of strings that combine the company's catchPhrase and bs
  const companyJargon = data.map(user => '$
  {user.company.catchPhrase} ${user.address.bs}
  ';);